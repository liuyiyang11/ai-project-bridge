...
def test_fake_codex_app_server_completes_full_async_bridge_chain(tmp_path):
    ...
    supervisor, queue, store = _make_supervisor(tmp_path, client_factory=lambda **kwargs: _attach(fake, kwargs))
    ...
    assert [name for name, _ in fake.requests] == ["initialize", "model/list", "thread/start", "turn/start"]
    assert "turn/completed" in [event["method"] for event in fake.notifications]
